...
class AppStore extends Component {
  state = {activeTabId: 'SOCIAL', searchInput: ''}
  ...
  render() {
    ...
    const filteredList = appsList.filter(each => each.category === activeTabId)
    const secondFilteredList = filteredList.filter(each =>
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="app-ul-con">
          {secondFilteredList.map(each => (
            <AppItem each={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
